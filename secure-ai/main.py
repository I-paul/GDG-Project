...
def initialize_cameras():
    """Initialize all camera sources and processing threads."""
    global active_streams, caps, frame_queues, result_queues, processing_threads
...
def main():
    """Main function to run the multi-camera face tracking system."""
    try:
        initialize_cameras()
        ...
        while True:
            ...
            for source_id, cap in caps.items():
                ...
                # Ensure the window is created before displaying or moving it
                window_name = f"Camera {source_id}"
                ...
                try:
                    if not result_queues[source_id].empty():
                        ...
                        # Show the frame
                        cv2.imshow(window_name, frame_with_tracks)
                except queue.Empty:
                    pass
                except Exception as e:
...
if __name__ == "__main__":
    print("Starting Multi-camera face tracking system...")
    main()
